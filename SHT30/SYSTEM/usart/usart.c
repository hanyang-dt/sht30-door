...
void Usart2_SendNByte(u8 *Data,u8 length)
{
    ...
    for(i=0;i<length;i++)
    {
        ...
        while(USART_GetFlagStatus(USART2, USART_FLAG_TXE) == RESET);//等待消息发送完成
    }
}
//串口1中断服务程序
//注意,读取USARTx->SR能避免莫名其妙的错误   	
char RxCounter,RxBuffer[100];     //接收缓冲,最大USART_REC_LEN个字节.   //串口二
char RxCounter1,RxBuffer1[100];     //接收缓冲,最大USART_REC_LEN个字节. //串口一
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
